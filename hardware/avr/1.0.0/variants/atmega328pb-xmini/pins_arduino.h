...
/*
 * This is the pinmap for Atmel ATmega328PB Xplained Mini.
 *
 * The mEDBG component bridges Serial (D0/D1) to USB CDC and
 * allows device ISP programming through CMSIS-DAP.
 *
 * Please, check "ATmega328PB Xplained Mini User Guide" at http://www.atmel.com/tools/mega328pb-xmini.aspx?tab=documents
 *
 *
 *                                                         +-------+
 *                                                         | |USB| |
 ...
 * Push button SW0
 */
static const uint8_t SW0 = 24; // PB7

#define analogInputToDigitalPin(p)  ((p < 6) ? (p) + A0 : (p < 8) ? (p) - 6 + A6 : -1)
...
#define digitalPinToPCMSK(p)        (((p) <= 7) ? (&PCMSK2) : (((p) <= 13) ? (&PCMSK0) : (((p) <= 19) ? (&PCMSK1) : ((p) < NUM_DIGITAL_PINS) ? (&PCMSK3) : ((uint8_t *)0))))
...
const uint8_t PROGMEM digital_pin_to_timer_PGM[] =
{
	...
	NOT_ON_TIMER,
	NOT_ON_TIMER,
	NOT_ON_TIMER, /* 0 - port E */
	NOT_ON_TIMER,
	NOT_ON_TIMER,
	NOT_ON_TIMER,
	NOT_ON_TIMER  /* 24, Push button SW0 */
};
